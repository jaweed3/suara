<script lang="ts">
  ...
  import {userFollows, calculateTrustScore} from "src/engine"
  ...
  const trustScore = calculateTrustScore(pubkey)
...
<div on:click|stopPropagation>
  <Popover triggerType="mouseenter" opts={{hideOnClick: true}} placement="right">
    <div slot="trigger">
      <WotScore {accent} score={trustScore} max={100} />
    </div>
    <div slot="tooltip" class="p-4">
      ...
      <div class="mt-4 flex items-center gap-2">
        <Link modal class="flex items-center gap-1" href="/help/web-of-trust">
          Kepercayaan: {Math.round(trustScore)}%
