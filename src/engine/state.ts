 ...
 */
export function calculateTrustScore(pubkey: string): number {
  // Base Welshman WoT score normalized to 0-100 range
  const baseWot = getUserWotScore(pubkey)
  const maxWot = getMaxWot() || 1
...
export const trustLabelStore = derived(
  [pubkey],
  ([_pk]) => calculateTrustScore
)
    ...
    ([
      ...
      return cached({
        ...
        getValue: ([e, strict = false]: [e: HashedEvent, strict?: boolean]) => {
          ...
          const trustScore = calculateTrustScore(e.pubkey)
          
          // Enhanced WOT: trust is 0-100 scale now
          const okTrusted = trustScore >= getSetting("min_wot_score") * (.9 + Math.random() * .1)
